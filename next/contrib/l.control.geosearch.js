...
L.Control.GeoSearch = L.Control.extend({
    ...
    _showLocation: function (location) {
        ...
        this._map.on('click', this.removeResults, this);
	...
	for (i=0;i<results.length;i++)
	{
	    ...
	    var li = $('<li>', {
                ...
                click: function(e) {
                    geosearch._showLocation($(e.target).data('loc'));
                }
            });
            li.data('loc', results[i]);
	    $(this._resultslist).append(li);
	}
        $(this._resultslist).fadeIn('slow');
    },

    removeResults: function() {
        this._map.removeEventListener('click', this.removeResults);
        $(this._resultslist).fadeOut('slow');
    },
